    ...
    <section className="relative overflow-hidden border-t border-white/5 bg-black py-20 sm:py-28">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="max-w-2xl" data-aos="fade-up">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#29B9F2]">What Sets Us Apart</p>
        ...
        <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 lg:auto-rows-[220px]">
          <div className="lg:col-span-2 lg:row-span-2" data-aos="fade-up">
            <TiltCard className="h-full">
              <div className="group relative h-full min-h-[280px] overflow-hidden rounded-3xl border border-white/10">
                <Image
                  src={featured.image}
                  alt={featured.title}
                  fill
                ...
                <div className="absolute inset-x-0 bottom-0 p-7">
                  ...
                  <h3 className="mt-4 text-2xl font-semibold text-white">{featured.title}</h3>
                  <p className="mt-2 max-w-sm text-sm leading-relaxed text-white/60">{featured.description}</p>
                </div>
              </div>
            </TiltCard>
          </div>
          ...
          {rest.map((item, i) => {
            ...
            return (
              <div key={item.title} data-aos="fade-up" data-aos-delay={i * 80}>
                <SpotlightCard className="h-full rounded-3xl border border-white/10 bg-white/[0.03] p-6 transition hover:border-white/20">
